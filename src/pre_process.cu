...
__constant__ float stddev[3] = {1.0, 1.0, 1.0};
...
ImgPreProcess::ImgPreProcess(const int width, const int height, const int channels, const float resize_scale, const int padding_divider) {
    ...
    width_resize_ = (int)(width_ * resize_scale); 
    height_resize_ = (int)(height_ * resize_scale);
    ...
    size_t bytes = width_ * height_ * channels_ * sizeof(float);
    fprintf(stderr, "Allocating pinned host memory of size: %zu bytes\n", bytes);
    std::cerr << "AFTER RESIZE: width: " <<  width_resize_ <<   " height: " << height_resize_ << " channel: " << channels_ << std::endl;
...
std::vector<float> ImgPreProcess::img_pre_processing(const std::vector<unsigned char*>& img, void *stream) {
    ...
    for (size_t imgid=0; imgid<img.size(); ++imgid) {
        ...
        num_blocks = (width_ * height_ + threads_per_block - 1) / threads_per_block;
        permute_img_channel_kernel<<<num_blocks, threads_per_block, 0, _stream>>>(
            img_d, width_, height_, channels_);
