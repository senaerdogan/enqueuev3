...
std::vector<std::vector<std::string>> parse_input_info(const std::string& input_pth, size_t num_inputs, int bias) {
    ...
    while (std::getline(info_file, info_str) && infos_vec.size()<num_inputs) {
        ...
        while (info_str.find(';') != std::string::npos) {
            ...
            std::string temp = info_str.substr(0, length);
            info_.push_back(temp);
...
static void visualize(const std::vector<unsigned char*> images, const UniAD::KernelInput& input_instance, const UniAD::KernelOutput& output_instance, const std::string& save_path,
    ...
    std::cerr << pred_bbox.size() << std::endl;
    ...
    for (size_t icamera = 0; icamera < images.size(); ++icamera) {
        ...
        /*std::cout << "CAM " << icamera << ": trying to draw " << pred_bbox.size() << " boxes" << std::endl;
        for (const auto& box : pred_bbox) {
            // Box: [x, y, z, dx, dy, dz, yaw, vx, vy]
            std::cout << "  box center: (" << box[0] << ", " << box[1] << ", " << box[2] << ")" << std::endl;
...
int main(int argc, char** argv) {
    ...
    std::string engine_p1_path = "./engine_part1";
    std::string engine_p2_path = "./engine_part2";
    std::string engine_p3_path = "./engine_part3";
    if (argc == 7) {
        const std::string mmdeploy_plugin_path = argv[6]; 
        void* so_handle_mmdeploy = dlopen(mmdeploy_plugin_path.c_str(), RTLD_NOW);
    }
    ...
    std::shared_ptr<UniAD::Kernel> kernel = create_kernel(engine_p1_path, engine_p2_path, engine_p3_path);
    if (kernel == nullptr) {
    ...
    std::cerr << "ORIGINAL WIDTH: " << original_width <<  " HEIGHT: " << original_height << " CHANNEL: " << original_channel << std::endl;
    free_images(images_dummy);
    std::shared_ptr<ImgPreProcess> pre_processor = std::make_shared<ImgPreProcess>(original_width, original_height, original_channel, 1, 32);
